...
import { SunStar } from './Illustrations';
...
const StageCard: React.FC<StageCardProps> = ({ age, title, whatHappens, careTips, funFact, delay = 0 }) => {
    const [ref, inView] = useInView({ triggerOnce: true });
    
    return (
        <div ref={ref} className={`bg-cream p-8 rounded-2xl transition-all duration-700 ease-out group hover:shadow-xl hover:scale-[1.02] ${inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`} style={{transitionDelay: `${delay}ms`}}>
            <div className="grid md:grid-cols-3 gap-8 items-start">
                <div className="flex flex-col items-center text-center md:col-span-1">
                    <SunStar className="w-40 h-40 text-navy-blue transition-transform duration-500 group-hover:rotate-12"/>
                </div>
    ...
    return (
        <section id="etapas" className="py-24">
            <div className="text-center mb-16">
                ...
                <h3 className="font-display text-4xl md:text-5xl text-coral-dark">y qué necesita en cada etapa</h3>
                <p className="mt-4 max-w-3xl mx-auto text-lg">
                    Tu piel no es igual todos los días ni todos los años. Crece contigo, cambia cuando creces, y necesita diferentes cuidados según tu edad. Aquí puedes conocer qué pasa en tu piel y cómo cuidarla según tu momento.
                </p>
            </div>
